void main() {
  ...
  print('   Sunday 12:13 AM  → Business Date: Saturday (still in Saturday business day)');  
  print('   Sunday 12:30 AM  → Business Date: Saturday (end of Saturday business day)');
  print('   Sunday 4:00 AM   → Business Date: Sunday (start of Sunday business day)');
}
...
void testBusinessDate(DateTime dateTime) {
  ...
  print('   ${formatDateTime(dateTime)} → Business Date: ${formatDate(businessDate)} (${weekdayName(businessDate.weekday)})');
}

/// Test close day offset calculation
void testCloseDayOffset(int closeMinutes) {
  final offset = (closeMinutes >= 1440) ? 1 : 0;
  final timeStr = '${closeMinutes ~/ 60}:${(closeMinutes % 60).toString().padLeft(2, '0')}';
  print('   $closeMinutes min ($timeStr) → Offset: $offset ${offset == 1 ? "(next day)" : "(same day)"}');
}

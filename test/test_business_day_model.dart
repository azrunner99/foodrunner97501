...
void main() {
  ...
  final testTimes = [
    ...
    DateTime(2025, 9, 15, 0,
        13), // Sunday 12:13 AM - should be open (your reported issue)
  ...
  for (final testTime in testTimes) {
    ...
    print(
        '  Open Minutes: $openMinutes (${openMinutes ~/ 60}:${(openMinutes % 60).toString().padLeft(2, '0')})');
    print('  Close Minutes: $closeMinutes (raw)');

    if (closeDayOffset == 1) {
      final actualCloseMinutes = closeMinutes - 1440;
      print(
          '  Actual Close: $actualCloseMinutes (${actualCloseMinutes ~/ 60}:${(actualCloseMinutes % 60).toString().padLeft(2, '0')} next day)');
    }
  }
  ...
  print(
      '- Sunday 4:00 AM: Business day boundary (start of Sunday business day)');
}

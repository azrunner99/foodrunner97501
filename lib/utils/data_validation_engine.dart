...
class DataValidationEngine {
  // Optional toggle for offloading heavy analyses to another isolate.
  // Intentionally disabled by default to preserve current behavior.
  static const bool kOffloadHeavyAnalysis = false;
  ...
  static Future<ValidationReport> validateAllData({AppState? appState}) async {
    ...
    try {
      ...
      final integrityResults =
          await _performDataIntegrityChecks(shifts, servers);
    ...
    final consistencyResults =
      await _validateDataConsistency(shifts, servers, state.profiles);
      ...
      final recommendations =
          _generateValidationRecommendations(validationReport);
  ...
  static Future<List<PerformanceAnomaly>> _detectPerformanceAnomalies(
    ...
    for (final server in servers) {
      ...
      final efficiencyData =
          _calculateEfficiencyTrends(server.id, serverShifts);
  ...
  static Future<List<StatisticalOutlier>> _analyzeStatisticalOutliers(
    ...
    if (kOffloadHeavyAnalysis) {
      // Offload heavy statistics to an isolate. Payload must be sendable (Map/List/primitives).
      final payload = <String, dynamic>{
        'shifts': shifts.map((s) => s.toMap()).toList(),
        'servers': servers.map((s) => s.toMap()).toList(),
      };
      final resultMaps = await compute(_computeOutliersIsolate, payload);
      return resultMaps
          .map((m) => StatisticalOutlier(
                ...
                detectedDate: DateTime.parse(m['detectedDate'] as String),
              ))
          .toList();
    } else {
      ...
      for (final server in servers) {
        ...
        final modifiedZOutliers =
            _detectModifiedZScoreOutliers(serverData, server);
...
// (moved to top-level at end of file)
  ...
  static Future<ConsistencyResults> _validateDataConsistency(
    ...
    final expectedTotal = serverShifts.fold<int>(
      0, (sum, shift) => sum + (shift.counts[server.id] ?? 0));
    final profile = profiles[server.id] ?? ServerProfile();
  ...
  static Future<List<UnusualPattern>> _detectUnusualPatterns(
    ...
    for (final server in servers) {
      ...
      final performanceChanges =
          _detectPerformanceChanges(server.id, serverShifts);
  ...
  static Map<DateTime, double> _calculateDailyPerformance(
      String serverId, List<ShiftRecord> shifts) {
    final dailyPerformance = <DateTime, double>{};

    for (final shift in shifts) {
      final date =
          DateTime(shift.start.year, shift.start.month, shift.start.day);
  ...
  static List<double> _calculateEfficiencyTrends(
      String serverId, List<ShiftRecord> shifts) {
  ...
  static List<DataIssue> _checkMissingData(
      List<ShiftRecord> shifts, List<Server> servers) {
  ...
  static List<DataIssue> _checkDataConsistency(List<ShiftRecord> shifts) {
    ...
    for (final shift in shifts) {
      ...
      for (final entry in shift.counts.entries) {
        if (entry.value > 100) {
          // Assuming 100+ runs in a shift is unusual
          issues.add(DataIssue(
    ...
    for (int i = 1; i < sortedShifts.length; i++) {
      ...
      if (current.start.difference(previous.start).inHours < 1) {
        issues.add(DataIssue(
          ...
          description:
              'Shifts very close together: ${previous.start} and ${current.start}',
          severity: 'medium',
          detectedDate: DateTime.now(),
        ));
      }
  ...
  static List<double> _extractServerPerformanceData(
      String serverId, List<ShiftRecord> shifts) {
    return shifts
  ...
  static List<StatisticalOutlier> _detectZScoreOutliers(
      List<double> data, Server server) {
    final outliers = <StatisticalOutlier>[];
  ...
  /// Detect IQR outliers
  static List<StatisticalOutlier> _detectIQROutliers(
      List<double> data, Server server) {
    final outliers = <StatisticalOutlier>[];
  ...
  static List<StatisticalOutlier> _detectModifiedZScoreOutliers(
      List<double> data, Server server) {
    final outliers = <StatisticalOutlier>[];
  ...
  static List<UnusualPattern> _analyzeWeeklyPatterns(
      String serverId, List<ShiftRecord> shifts) {
    final patterns = <UnusualPattern>[];
    ...
    for (final entry in weeklyData.entries) {
      ...
      if (cv > 0.8) {
        ...
        patterns.add(UnusualPattern(
          ...
          description:
              'High performance variability on $dayName (CV: ${cv.toStringAsFixed(2)})',
  ...
  /// Detect sudden performance changes
  static List<UnusualPattern> _detectPerformanceChanges(
      String serverId, List<ShiftRecord> shifts) {
    final patterns = <UnusualPattern>[];
    ...
    for (int i = windowSize; i < sortedDates.length - windowSize; i++) {
      ...
      final beforeAvg = beforeWindow
              .map((d) => dailyPerformance[d]!)
              .reduce((a, b) => a + b) /
          windowSize;
      final afterAvg =
          afterWindow.map((d) => dailyPerformance[d]!).reduce((a, b) => a + b) /
              windowSize;
      ...
      if (changePercent > 50) {
        ...
        patterns.add(UnusualPattern(
          serverId: serverId,
          patternType:
              changePercent > 0 ? 'sudden_improvement' : 'sudden_decline',
          description:
              'Sudden ${changePercent > 0 ? 'improvement' : 'decline'} of ${changePercent.toStringAsFixed(1)}% detected',
  ...
  static List<UnusualPattern> _analyzeShiftPatterns(
      String serverId, List<ShiftRecord> shifts) {
    final patterns = <UnusualPattern>[];
    ...
    for (final entry in hourCounts.entries) {
      if ((entry.key < 6 || entry.key > 23) && entry.value > 2) {
        patterns.add(UnusualPattern(
          ...
          description:
              'Multiple shifts at unusual hour: ${entry.key}:00 (${entry.value} shifts)',
          severity: 'medium',
          confidence: 0.8,
          detectedDate: DateTime.now(),
  ...
  static String _generateAnomalyDescription(
      DataOutlier outlier, String serverName) {
  ...
  static List<String> _identifyPossibleCauses(
      DataOutlier outlier, List<ShiftRecord> shifts) {
    final causes = <String>[];
  ...
  static List<ValidationRecommendation> _generateValidationRecommendations(
      ValidationReport report) {
    final recommendations = <ValidationRecommendation>[];

    // High severity anomalies
    final criticalAnomalies =
        report.anomalies.where((a) => a.severity == 'critical').length;
    if (criticalAnomalies > 0) {
      recommendations.add(ValidationRecommendation(
        ...
        description:
            '$criticalAnomalies critical performance anomalies require immediate attention',
        actionItems: [
    ...
    if (report.integrityResults.duplicateEntries.isNotEmpty) {
      recommendations.add(ValidationRecommendation(
        ...
        description:
            '${report.integrityResults.duplicateEntries.length} duplicate entries detected',
        actionItems: [
    ...
    if (report.consistencyResults.totalDiscrepancies.isNotEmpty) {
      recommendations.add(ValidationRecommendation(
        ...
        description:
            '${report.consistencyResults.totalDiscrepancies.length} data discrepancies found',
        actionItems: [
...
// Top-level function for compute() offload — must not capture closures.
List<Map<String, dynamic>> _computeOutliersIsolate(Map<String, dynamic> payload) {
  final shiftsMaps = (payload['shifts'] as List).cast<Map<String, dynamic>>();
  final serversMaps = (payload['servers'] as List).cast<Map<String, dynamic>>();

  final shifts = shiftsMaps.map((m) => ShiftRecord.fromMap(m)).toList();
  final servers = serversMaps.map((m) => Server.fromMap(m)).toList();

  final out = <Map<String, dynamic>>[];
  const minimumDataPoints = DataValidationEngine._minimumDataPoints;
  const outlierThreshold = DataValidationEngine._outlierThreshold;

  List<double> extract(String serverId) => shifts
      .where((s) => s.counts.containsKey(serverId))
      .map((s) => (s.counts[serverId] ?? 0).toDouble())
      .toList();

  List<Map<String, dynamic>> detectZ(
      List<double> data, Server server, double threshold) {
    final results = <Map<String, dynamic>>[];
    if (data.length < minimumDataPoints) return results;
    final mean = data.reduce((a, b) => a + b) / data.length;
    final variance =
        data.map((x) => math.pow(x - mean, 2)).reduce((a, b) => a + b) /
            data.length;
  ...
  List<Map<String, dynamic>> detectIqr(List<double> data, Server server) {
    final results = <Map<String, dynamic>>[];
    if (data.length < minimumDataPoints) return results;
  ...
  List<Map<String, dynamic>> detectModZ(List<double> data, Server server) {
    final results = <Map<String, dynamic>>[];
    if (data.length < minimumDataPoints) return results;
    final sorted = List<double>.from(data)..sort();
  ...
  for (final server in servers) {
    final data = extract(server.id);
    if (data.length < minimumDataPoints) continue;
    out.addAll(detectZ(data, server, outlierThreshold));
    out.addAll(detectIqr(data, server));
    out.addAll(detectModZ(data, server));
  }

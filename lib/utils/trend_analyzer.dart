...
class TrendAnalyzer {
  ...
  static PerformanceTrendAnalysis analyzePerformanceTrends({
    ...
    final dailyTrends =
        _calculateDailyTrends(serverId, shifts, analysisWindowDays);
  ...
  static PeriodComparison comparePerformancePeriods({
    ...
    final period1Data =
        _extractPeriodData(serverId, shifts, period1Start, period1End);
    final period2Data =
        _extractPeriodData(serverId, shifts, period2Start, period2End);

    final comparison = _comparePeriods(period1Data.data, period2Data.data);
    final significanceTest =
        _testStatisticalSignificance(period1Data.data, period2Data.data);
  ...
  static SchedulingOptimization optimizeScheduling({
    ...
    final shiftPerformance =
        _analyzeShiftTypePerformance(serverId, historicalShifts);

    // Identify optimal shift patterns
    final optimalPatterns =
        _identifyOptimalShiftPatterns(shiftPerformance, seasonalAnalysis);

    // Generate scheduling recommendations
    final recommendations =
        _generateSchedulingRecommendations(optimalPatterns, seasonalAnalysis);
  ...
  static PerformanceVelocity calculatePerformanceVelocity(
    ...
    // Sort trends by date
    final sortedTrends = List<PerformanceTrend>.from(trends)
  ...
  static List<DailyTrend> _calculateDailyTrends(
      String serverId, List<ShiftRecord> shifts, int windowDays) {
    ...
    for (int i = 0; i < windowDays; i++) {
      ...
      final totalRuns = dayShifts.fold<int>(
  ...
  static RollingAverages _calculateRollingAverages(
  ...
  static List<double> _calculateRollingAverage(
  ...
  static TrendDirection _analyzeTrendDirection(List<DailyTrend> trends) {
    ...
    final recentScores =
        trends.skip(trends.length - 5).map((t) => t.score).toList();
  ...
  static double _calculateLinearRegressionSlope(
  ...
  static PerformancePredictions _predictFuturePerformance(
  ...
  static double _calculateLinearRegressionIntercept(
      List<double> x, List<double> y) {
    if (x.length != y.length || x.isEmpty) return 0.0;
  ...
  static List<AnomalyPoint> _detectAnomalies(List<DailyTrend> trends) {
    ...
    for (int i = 0; i < trends.length; i++) {
      final zScore = standardDeviation > 0
          ? (trends[i].score - mean) / standardDeviation
          : 0.0;
      ...
      if (zScore.abs() > 2.0) {
        ...
        anomalies.add(AnomalyPoint(
          ...
          zScore: zScore,
          type:
              zScore > 0 ? AnomalyType.unusuallyHigh : AnomalyType.unusuallyLow,
        ));
      }
    }
  ...
  static double _calculateTrendConfidence(
      List<DailyTrend> trends, TrendDirection direction) {
    ...
    double confidence =
        math.min(1.0, dataPoints / 30.0); // More data = higher confidence
  ...
  static Map<String, List<ShiftRecord>> _groupShiftsByMonth(
      String serverId, List<ShiftRecord> shifts, int monthsToAnalyze) {
  ...
  static Map<int, double> _analyzeDayOfWeekPatterns(
  ...
  static Map<String, double> _analyzeMonthlyPatterns(
      Map<String, List<ShiftRecord>> monthlyData) {
    // Analyze monthly performance patterns
    return monthlyData.map((month, shifts) {
      final totalRuns = shifts.fold<int>(
          0,
          (sum, shift) =>
  ...
  static PerformancePeaks _identifyPerformancePeaks(
      Map<String, List<ShiftRecord>> monthlyData) {
    final monthlyTotals = _analyzeMonthlyPatterns(monthlyData);

    if (monthlyTotals.isEmpty) {
      return PerformancePeaks(
          bestMonth: '', worstMonth: '', peakScore: 0.0, lowScore: 0.0);
    }
  ...
  static Map<String, double> _calculateSeasonalAdjustments(
      Map<String, List<ShiftRecord>> monthlyData) {
    final monthlyTotals = _analyzeMonthlyPatterns(monthlyData);
  ...
  static PeriodData _extractPeriodData(
      String serverId, List<ShiftRecord> shifts, DateTime start, DateTime end) {
    ...
    final totalRuns = periodShifts.fold<int>(
        0, (sum, shift) => sum + (shift.counts[serverId] ?? 0));
    final avgRunsPerShift =
        periodShifts.isNotEmpty ? totalRuns / periodShifts.length : 0.0;
  ...
  static Map<String, double> _comparePeriods(
      Map<String, double> period1, Map<String, double> period2) {
    ...
    for (final key in period1.keys) {
      if (period2.containsKey(key)) {
        ...
        final percentChange =
            period1[key]! > 0 ? (change / period1[key]!) * 100 : 0.0;
  ...
  static StatisticalSignificance _testStatisticalSignificance(
      Map<String, double> period1, Map<String, double> period2) {
    // Simplified significance test
    final totalRuns1 = period1['totalRuns'] ?? 0.0;
    final totalRuns2 = period2['totalRuns'] ?? 0.0;

    final changePercent = totalRuns1 > 0
        ? ((totalRuns2 - totalRuns1) / totalRuns1).abs() * 100
        : 0.0;
  ...
  static Map<String, double> _analyzeShiftTypePerformance(
      String serverId, List<ShiftRecord> shifts) {
  ...
  static List<String> _identifyOptimalShiftPatterns(
      Map<String, double> shiftPerformance, SeasonalAnalysis seasonalAnalysis) {
  ...
  static List<String> _generateSchedulingRecommendations(
      List<String> optimalPatterns, SeasonalAnalysis seasonalAnalysis) {
    final recommendations = <String>[];

    if (optimalPatterns.isNotEmpty) {
      recommendations.add(
          'Schedule more ${optimalPatterns.first} shifts for optimal performance');
    }
  ...
  static double _calculateLongTermVelocity(List<PerformanceTrend> trends) {
    ...
    return (lastScore - firstScore) /
        trends.length; // Points per day over entire period
  }
  ...
  static VelocityCategory _categorizeVelocity(
      double shortTerm, double longTerm) {
    if (shortTerm > 1.0 && longTerm > 0.5)
      return VelocityCategory.rapidImprovement;
    if (shortTerm > 0.5 && longTerm > 0.2)
      return VelocityCategory.steadyImprovement;
    if (shortTerm.abs() < 0.2 && longTerm.abs() < 0.1)
      return VelocityCategory.stable;
    if (shortTerm < -0.5 && longTerm < -0.2)
      return VelocityCategory.steadyDecline;
    if (shortTerm < -1.0 && longTerm < -0.5)
      return VelocityCategory.rapidDecline;

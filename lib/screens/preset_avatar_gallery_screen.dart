...
class PresetAvatarGalleryScreen extends StatelessWidget {
  ...
  const PresetAvatarGalleryScreen(
      {super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> allPresetAvatars = List.generate(131,
        (i) => 'assets/avatars/image${(i + 1).toString().padLeft(3, '0')}.png');
    ...
    for (var profile in app.profiles.values) {
      if (profile.avatarPath != null &&
          ...
          app.profiles.keys.firstWhere((id) => app.profiles[id] == profile,
                  orElse: () => '') !=
              currentServerId) {
    ...
    final List<String> availableAvatars = allPresetAvatars
        .where((avatar) => !usedAvatars.contains(avatar))
        .toList();

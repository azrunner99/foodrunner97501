...
class ServerPerformanceScreenWorking extends StatefulWidget {
  const ServerPerformanceScreenWorking({super.key});

  @override
  State<ServerPerformanceScreenWorking> createState() =>
      _ServerPerformanceScreenWorkingState();
}

class _ServerPerformanceScreenWorkingState extends State<ServerPerformanceScreenWorking> {    
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Rich Performance Badges (like in the original image)
              _buildRichPerformanceBadges(performance, rank),
  ...
  Widget _buildRichPerformanceBadges(ServerPerformanceData performance, int rank) {
    final badges = <Widget>[];
    
    // Confidence Level Badge
    final confidenceLevel = _calculateConfidenceLevel(performance);
    badges.add(_buildBadge(
      '${_getConfidenceIcon(confidenceLevel)} $confidenceLevel',
      _getConfidenceColor(confidenceLevel),
      Icons.info_outline,
    ));

    // Performance Tier Badge
    final performanceTier = _calculatePerformanceTier(performance.performanceScore);
    badges.add(_buildBadge(
      '${_getTierIcon(performanceTier)} $performanceTier ${performance.performanceScore.toStringAsFixed(0)}%',
      _getTierColor(performanceTier),
      Icons.star,
    ));

    // Trend Badge
    final trend = _calculateTrend(performance);
    badges.add(_buildBadge(
    ...
    // Consistency Badge
    final consistency = _calculateConsistency(performance);
    badges.add(_buildBadge(
      '${_getConsistencyIcon(consistency)} $consistency',
      _getConsistencyColor(consistency),
      Icons.timeline,
    ));

    // Status Badge
    final status = _calculateStatus(performance);
    badges.add(_buildBadge(
    ...
    // Action Badge
    final action = _calculateAction(performance);
    badges.add(_buildBadge(
  ...
  String _calculateConfidenceLevel(ServerPerformanceData performance) {
    if (performance.shiftsWorked < 3) return 'Very Low Confidence';
    if (performance.shiftsWorked < 7) return 'Low Confidence';
    if (performance.shiftsWorked < 15) return 'Moderate Confidence';
    return 'High Confidence';
  }

  String _calculatePerformanceTier(double score) {
  ...
  String _calculateTrend(ServerPerformanceData performance) {
    // Simple trend calculation based on performance score
    if (performance.performanceScore >= 80) return 'Improving';
    if (performance.performanceScore >= 60) return 'Stable';
    if (performance.performanceScore >= 40) return 'Declining';
    return 'Volatile';
  }

  String _calculateConsistency(ServerPerformanceData performance) {
    if (performance.metrics.consistencyScore >= 80) return 'High';
    if (performance.metrics.consistencyScore >= 60) return 'Moderate';
    if (performance.metrics.consistencyScore >= 40) return 'Low';
    return 'Very Low';
  }

  String _calculateStatus(ServerPerformanceData performance) {
    if (performance.performanceScore >= 80) return 'Excellent';
    if (performance.performanceScore >= 60) return 'Good';
    if (performance.performanceScore >= 40) return 'Needs Attention';
    if (performance.performanceScore >= 20) return 'Critical';
    return 'Emergency';
  }

  String _calculateAction(ServerPerformanceData performance) {
    if (performance.performanceScore >= 80) return 'Recognition Due';
    if (performance.performanceScore >= 60) return 'Monitor';
    if (performance.performanceScore >= 40) return 'Coaching Needed';
    return 'Immediate Action';
  }

  // Color and icon methods for badges
  Color _getConfidenceColor(String level) {
    switch (level) {
      case 'High Confidence': return Colors.green;
      case 'Moderate Confidence': return Colors.blue;
      case 'Low Confidence': return Colors.orange;
      case 'Very Low Confidence': return Colors.red;
  ...
  Color _getTrendColor(String trend) {
    switch (trend) {
      ...
      case 'Volatile': return Colors.purple;
      default: return Colors.grey;
    }
  }
  ...
  Color _getConsistencyColor(String consistency) {
    switch (consistency) {
      ...
      case 'Very Low': return Colors.red.shade900;
      default: return Colors.grey;
    }
  }
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'Emergency': return Colors.red.shade900;
      default: return Colors.grey;
    }
  }
  ...
  Color _getActionColor(String action) {
    switch (action) {
      case 'Recognition Due': return Colors.green;
      case 'Monitor': return Colors.blue;
      case 'Coaching Needed': return Colors.orange;
      case 'Immediate Action': return Colors.red;
      default: return Colors.grey;
    }
  }
  ...
  IconData _getConfidenceIcon(String level) {
    switch (level) {
      case 'High Confidence': return Icons.check_circle;
      case 'Moderate Confidence': return Icons.info;
      case 'Low Confidence': return Icons.warning;
      case 'Very Low Confidence': return Icons.error;
  ...
  IconData _getTierIcon(String tier) {
    switch (tier) {
      case 'Elite': return Icons.emoji_events;
      case 'Top Performer': return Icons.star;
      case 'Good': return Icons.thumb_up;
      case 'Average': return Icons.trending_flat;
      case 'Needs Improvement': return Icons.thumb_down;
  ...
  IconData _getTrendIcon(String trend) {
    switch (trend) {
      ...
      case 'Volatile': return Icons.waves;
      default: return Icons.help;
    }
  }

  IconData _getConsistencyIcon(String consistency) {
    switch (consistency) {
      case 'High': return Icons.timeline;
      case 'Moderate': return Icons.trending_flat;
      case 'Low': return Icons.trending_down;
      case 'Very Low': return Icons.warning;
      default: return Icons.help;
    }
  }
  ...
  IconData _getStatusIcon(String status) {
    switch (status) {
      ...
      case 'Good': return Icons.thumb_up;
      case 'Needs Attention': return Icons.warning;
      case 'Critical': return Icons.error;
      case 'Emergency': return Icons.priority_high;
      default: return Icons.help;
    }
  }
  ...
  IconData _getActionIcon(String action) {
    switch (action) {
      case 'Recognition Due': return Icons.emoji_events;
      case 'Monitor': return Icons.visibility;
      case 'Coaching Needed': return Icons.school;
      case 'Immediate Action': return Icons.priority_high;
      default: return Icons.help;
    }
  }

...
class _LevelColorDemoScreenState extends State<LevelColorDemoScreen> {
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildTierDemo('Beginner', [1, 2, 3, 4, 5], '1-5',
                            Colors.green.shade100, serverCounts),
  ...
  Widget _buildTierDemo(String tierName, List<int> levels, String range,
      Color backgroundColor, Map<int, int> serverCounts) {
  ...
  Widget _buildLevelBubblesGrid(List<int> levels, Map<int, int> serverCounts) {
    ...
    for (int i = 0; i < levels.length; i += 5) {
      List<int> rowLevels =
          levels.sublist(i, (i + 5 > levels.length) ? levels.length : i + 5);

      rows.add(Row(
        children: rowLevels
            .map((level) => Expanded(
                child: _buildLevelBubble(
                    level, levels.indexOf(level), levels.length, serverCounts)))
            .toList(),
      ));
  ...
  Widget _buildLevelBubble(
      int level, int index, int totalInTier, Map<int, int> serverCounts) {
    ...
    final xpLabel = xpRequired >= 1000
        ? '${(xpRequired / 1000).toStringAsFixed(1)}k'
        : '$xpRequired';
        ...
        children: [
          serverCount > 0
              ? Material(
                  ...
                  child: InkWell(
                    borderRadius:
                        BorderRadius.circular(22), // Half of the larger size
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) =>
                              ServerLevelDetailScreen(level: level),
                        ),
                      );
                    },
                    child: Container(

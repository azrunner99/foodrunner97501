...
class ServerPerformanceProfileScreen extends StatefulWidget {
  ...
  @override
  State<ServerPerformanceProfileScreen> createState() => _ServerPerformanceProfileScreenState();
}

class _ServerPerformanceProfileScreenState extends State<ServerPerformanceProfileScreen> {
  ...
  Future<void> _loadShiftHistory() async {
    ...
    print('[Profile Debug] Loading shift history for server ${widget.server.name} (ID: ${widget.server.id})');
    print('[Profile Debug] Total shifts in app.history: ${app.history.length}');
    
    // Get ALL historical shifts for this server
    _allServerShifts = app.history.where((shift) => 
        shift.counts.containsKey(widget.server.id) && 
        (shift.counts[widget.server.id] ?? 0) > 0
    ).toList();
    
    print('[Profile Debug] Found ${_allServerShifts.length} shifts for this server');
    print('[Profile Debug] Server ID exists in shift counts: ${app.history.map((s) => s.counts.containsKey(widget.server.id)).toList()}');
    ...
    if (app.history.isNotEmpty) {
      ...
      for (var shift in sampleShifts) {
        print('[Profile Debug] Sample shift: ${shift.start}, counts: ${shift.counts}, contains ${widget.server.id}: ${shift.counts.containsKey(widget.server.id)}');
      }
    }
    ...
    _recentShifts = _allServerShifts
        .where((shift) => shift.start.isAfter(DateTime.now().subtract(const Duration(days: 90))))
        .take(50)
        .toList()
      ..sort((a, b) => b.start.compareTo(a.start));
    
    // Date range
    final sortedShifts = _allServerShifts..sort((a, b) => a.start.compareTo(b.start));
  ...
  Future<void> _loadNPSData() async {
    ...
    try {
      ...
      if (npsServer.isNotEmpty) {
        ...
        final monthsToCheck = [
          202509, 202508, 202507, 202506, 202505, 202504, 202503
        ];
        ...
        for (final monthKey in monthsToCheck) {
          try {
            final reportData = await npsProvider.database.getMonthlyReport(npsServerId, monthKey);
            if (reportData != null) {
              reportsFound++;
              if (latestReportData == null) latestReportData = reportData;
              
              final monthStr = '${monthKey.toString().substring(0, 4)}-${monthKey.toString().substring(4)}';
              ...
              final checkAverage = allTimeChecks > 0 ? (allTimeSales / allTimeChecks) : 0.0;
              
              adminNpsData.add('📅 $monthStr Report:');
              if (allTimeNps != null) adminNpsData.add('   All-time NPS: ${(allTimeNps as num).toStringAsFixed(1)}%');
              if (threeMonthNps != null) adminNpsData.add('   3-month NPS: ${(threeMonthNps as num).toStringAsFixed(1)}%');
              if (oneMonthNps != null) adminNpsData.add('   1-month NPS: ${(oneMonthNps as num).toStringAsFixed(1)}%');
              adminNpsData.add('   All-time sales: \$${allTimeSales.toStringAsFixed(0)}');
              adminNpsData.add('   All-time checks: $allTimeChecks');
              adminNpsData.add('   💰 Check average: \$${checkAverage.toStringAsFixed(2)}');
              adminNpsData.add('');
        ...
        if (reportsFound == 0) {
          adminNpsData = ['📊 No admin-entered NPS reports found for ${widget.server.name}', 
                        '⚠️ Use Admin → NPS Data Entry to add monthly reports'];
        } else {
          adminNpsData.insert(1, '✅ Found $reportsFound admin-entered monthly reports');
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (app.shiftActive && (_currentShiftRuns > 0 || _currentPizookieRuns > 0))
        ...
        child: Column(
          ...
          children: [
            ...
            if (_earliestShift != null) 
              _buildStatRow('First shift', _earliestShift!.toString().substring(0, 16)),
            if (_latestShift != null) 
              _buildStatRow('Latest shift', _latestShift!.toString().substring(0, 16)),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            ...
            if (_npsMetrics['allTimeNps'] != null) 
              _buildStatRow('All-time NPS', '${(_npsMetrics['allTimeNps'] as num).toStringAsFixed(1)}%', Colors.blue),
            _buildStatRow('All-time sales', '\$${(_npsMetrics['allTimeSales'] as num).toStringAsFixed(0)}', Colors.green),
            _buildStatRow('All-time checks', '${_npsMetrics['allTimeChecks']}'),
            _buildStatRow('💰 Check average', '\$${(_npsMetrics['checkAverage'] as num).toStringAsFixed(2)}', Colors.orange),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.admin_panel_settings, color: Colors.purple[600], size: 24),
                const SizedBox(width: 8),
                const Text(
            ...
            ..._npsHistoryText.map((text) => Padding(
              ...
              child: Text(
                text,
                style: TextStyle(
                  fontSize: text.startsWith('📊') || text.startsWith('📅') || text.startsWith('✅') || text.startsWith('❌') || text.startsWith('⚠️') ? 14 : 13,
                  fontWeight: text.startsWith('📊') || text.startsWith('📅') ? FontWeight.bold : FontWeight.normal,
                ),
              ),
            )),
          ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildStatRow('Current food runs', '$_currentShiftRuns', Colors.orange),
            _buildStatRow('Current pizookie runs', '$_currentPizookieRuns'),
            _buildStatRow('Status', 'In progress', Colors.green),
            _buildStatRow('Started', '~${DateTime.now().subtract(const Duration(hours: 2)).toString().substring(0, 16)} (estimated)'),
          ],
        ),
      ),
    );
              ...
              ...widget.performance.insights.take(5).map((insight) => Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border(left: BorderSide(width: 4, color: Colors.amber[600]!)),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [

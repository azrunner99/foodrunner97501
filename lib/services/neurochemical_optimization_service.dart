...
class AchievementCluster {
  ...
  String? get nextAchievement =>
      isComplete ? null : achievementSequence[currentPosition];
  int? get nextXP => isComplete ? null : xpSequence[currentPosition];
  Duration? get nextTiming =>
      isComplete ? null : timingSequence[currentPosition];
}
...
class NeurochemicalOptimizationService {
  ...
  static void updateDopamineState(String serverId, int xpReward, AppState app) {
    ...
    optimizer.currentIntensity =
        (optimizer.currentIntensity * 0.7 + normalizedReward * 0.3)
            .clamp(0.0, 1.0);
  ...
  static WithdrawalIntervention? checkWithdrawalIntervention(
    ...
    if (timeSinceLastReward.inMinutes >= 10 &&
        optimizer.currentPhase != DopaminePhase.withdrawal) {
    ...
    if (lastIntervention != null &&
        DateTime.now().difference(lastIntervention).inMinutes < 5) {
  ...
  static Map<String, dynamic> analyzeDopaminePatterns(String serverId,
      {AppState? app}) {
    final optimizer = _getOrCreateOptimizer(serverId);
    ...
    final baseAnalysis = {
      ...
      'timeSinceLastReward':
          DateTime.now().difference(optimizer.lastReward).inMinutes,
    ...
    if (app != null) {
      ...
      if (profile != null) {
        ...
        final shiftVsCareerRatio = profile.bestShiftRuns > 0
  ...
  static void _advanceDopaminePhase(DopamineOptimizer optimizer, int xpReward) {
    switch (optimizer.currentPhase) {
      ...
      case DopaminePhase.satisfaction:
        ...
        optimizer.currentPhase = xpReward >= 50
            ? DopaminePhase.satisfaction
            : DopaminePhase.withdrawal;
        break;
      case DopaminePhase.withdrawal:
        optimizer.currentPhase = DopaminePhase.craving;
        break;
  ...
  static WithdrawalIntervention _createIntervention(
    ...
    switch (timing) {
      case InterventionTiming.predictive:
        message =
            "🎯 PERFECT TIMING! Your brain is ready for something amazing!";
      ...
      case InterventionTiming.recovery:
        message = "🌈 RECOVERY MODE! Smooth climb back to flow!";
        priority = FeedbackPriority.medium;
        xpBonus = 30;
        context = {'type': 'recovery', 'phase': 'rebuilding'};
        break;
  ...
  static Map<String, dynamic> _calculateContextualTimingAdjustments(
    ...
    return {
      ...
      'optimalTiming':
          optimalTiming.clamp(30, 1800), // 30 seconds to 30 minutes
    };
  }
  ...
  static void cleanupExpiredData() {
    ...
    _activeClusters.removeWhere(
        (c) => c.isComplete || now.difference(c.startTime).inMinutes > 10);

    // Clean old intervention records
    _lastInterventions
        .removeWhere((key, value) => now.difference(value).inHours > 1);
  }
}

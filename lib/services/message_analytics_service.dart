...
class MessageAnalyticsService {
  ...
  static MessageAnalytics analyzeMessagePerformance(
    ...
    for (final profile in profiles.values) {
      for (final entry in profile.messageEngagement.entries) {
        ...
        if (lastUsed != null) {
          if (metrics.lastShown == null ||
              lastUsed.isAfter(metrics.lastShown!)) {
            metrics.lastShown = lastUsed;
          }
          if (metrics.firstShown == null ||
              lastUsed.isBefore(metrics.firstShown!)) {
    ...
    analytics.totalShows =
        messageStats.values.fold(0, (sum, m) => sum + m.totalShows);
    analytics.avgEngagementGlobal =
        _calculateGlobalEngagement(messageStats.values);
  ...
  static Map<MessageCategory, List<String>> getTopPerformingMessages(
    ...
    for (final entry in categorizedMessages.entries) {
      ...
      topPerformers[category] =
          messages.take(topCount).map((m) => m.message).toList();
    }
  ...
  static List<String> getUnderperformingMessages(MessageAnalytics analytics,
      {double engagementThreshold = 0.3}) {
    return analytics.messageMetrics.values
        .where(
            (m) => m.avgEngagement < engagementThreshold && m.totalShows >= 5)
        .map((m) => m.message)
        .toList();
  }
  ...
  static VarietyOptimization generateOptimizationRecommendations(
    ...
    optimization.recommendedCategoryBalance =
        _calculateOptimalCategoryBalance(analytics);
  ...
  static MessageCategory _categorizeMessage(String message) {
    ...
    if (lower.contains('win') ||
        ...
        lower.contains('dominating')) {
      return MessageCategory.competitiveBased;
    }

    if (lower.contains('perfect') ||
        lower.contains('flawless') ||
        lower.contains('precision') ||
        lower.contains('mastery') ||
        lower.contains('excellence')) {
  ...
  static List<String> _extractEngagementPatterns(
      List<MessageMetrics> highEngagement) {
  ...
  static Map<MessageCategory, double> _calculateOptimalCategoryBalance(
      MessageAnalytics analytics) {
...
class VarietyOptimization {
  ...
  List<String> getRecommendations() {
    ...
    if (staleMessages.isNotEmpty) {
      recommendations
          .add('Consider retiring ${staleMessages.length} stale messages');
    }

    if (overusedMessages.isNotEmpty) {
      recommendations.add(
          'Reduce frequency of ${overusedMessages.length} overused messages');
    }

    if (successPatterns.isNotEmpty) {
      recommendations.add(
          'High-engagement patterns found: ${successPatterns.take(3).join(", ")}');
    }

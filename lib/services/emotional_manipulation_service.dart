...
class EmotionalManipulationService {
  ...
  static ServerArchetype analyzeAndUpdateArchetype(
    ...
    final isUnderperforming =
        careerLevel == 'veteran' && shiftVsCareerRatio < 0.6;
    ...
    if (isUnderperforming &&
        (careerLevel == 'legend' || careerLevel == 'veteran')) {
      // Elite career performers having an off day
      newArchetype =
          ServerArchetype.phoenix; // Using phoenix as "fallen legend" recovery
    } else if (isExceeding && careerLevel == 'rookie') {
      // New talents breaking out
      newArchetype =
          ServerArchetype.eager; // Eager talent exceeding expectations
      ...
      if (profile.shiftsAsMvp >= 15 && currentRuns >= 20) {
        ...
        final isInDecline =
            shiftVsCareerRatio < 0.5 && careerLevel == 'veteran';

        if (comebackCount >= 3) {
          newArchetype = ServerArchetype.phoenix;
        } else if (isInDecline) {
          newArchetype =
              ServerArchetype.ghost; // Using ghost as "declining veteran"
  ...
  static AchievementNarrative generateAchievementNarrative(
      String serverId, ServerArchetype archetype, AppState app,
    ...
    final currentMoment =
        _generateCurrentMoment(arcType, achievementContext, personalData);
    final futurePromise =
        _generateFuturePromise(arcType, archetype, personalData);
  ...
  static FailureRecoveryStrategy generateFailureRecovery(
    ...
    switch (trigger) {
      case EmotionalTrigger.shame:
        ...
        reframingStatement =
            "This setback is data, not defeat. Champions analyze and adapt.";
      ...
      case EmotionalTrigger.fear:
        ...
        reframingStatement =
            "Fear means you care. That passion is your superpower.";
        actionCall = "Channel that energy into unstoppable action!";
        identityReinforcement =
            "You've overcome fear before - this is just another chapter.";
        urgencyLevel = 0.7;
        break;
      ...
      case EmotionalTrigger.inadequacy:
        ...
        reframingStatement =
            "Growth happens in the struggle. You're exactly where you need to be.";
        actionCall = "Prove to yourself what you're truly capable of!";
        identityReinforcement =
            "Your potential is limitless - this moment is proof.";
        urgencyLevel = 0.6;
        break;

      case EmotionalTrigger.nostalgia:
        motivationalMessage =
            _generateNostalgiaRecoveryMessage(archetype, serverId, app);
        reframingStatement =
            "Your best days aren't behind you - they're being written now.";
        actionCall = "Create a new legendary moment starting right now!";
        identityReinforcement =
            "Legends aren't made by past glory - they're forged in present action.";
        urgencyLevel = 0.9;
        break;

      default:
        motivationalMessage =
            "Every champion faces setbacks. What matters is how you respond.";
  ...
  static List<String> generateEmotionalTriggers(
      String serverId, EmotionalTrigger trigger, AppState app,
  ...
  static NarrativeArc _selectNarrativeArc(
      ServerArchetype archetype, String serverId, AppState app) {
    final profile = app.profiles[serverId];
    final currentRuns = app.currentCounts[serverId] ?? 0;
  ...
  static Map<String, dynamic> _gatherPersonalData(
      String serverId, AppState app) {
    final profile = app.profiles[serverId];
  ...
  static String _getArchetypeName(ServerArchetype archetype) {
    switch (archetype) {
      ...
      case ServerArchetype.reliable:
        return "Steady Champion";
      case ServerArchetype.competitive:
        return "Fierce Competitor";
      case ServerArchetype.speedster:
        return "Lightning Runner";
      case ServerArchetype.methodical:
        return "Precision Master";
      case ServerArchetype.veteran:
        return "Seasoned Expert";
      case ServerArchetype.champion:
      ...
      case ServerArchetype.master:
        return "Grand Master";
      case ServerArchetype.phoenix:
        return "Phoenix Warrior";
      case ServerArchetype.titan:
        return "Unstoppable Titan";
      case ServerArchetype.ghost:
        return "Mysterious Force";
    }
  }

  /// Generate current chapter description
  static String _generateCurrentChapter(
      NarrativeArc arcType, String serverId, AppState app) {
    final currentRuns = app.currentCounts[serverId] ?? 0;
  ...
  static List<String> _generatePastEvents(String serverId, AppState app) {
    ...
    if (profile.shiftsAsMvp >= 1) {
      events.add(
          "Earned MVP honors ${profile.shiftsAsMvp} time${profile.shiftsAsMvp > 1 ? 's' : ''}");
    }
    if (profile.achievements.length >= 5) {
      events.add(
          "Unlocked ${profile.achievements.length} achievements through skill and persistence");
    }
  ...
  static EmotionalTrigger _selectPrimaryEmotion(
      NarrativeArc arcType, ServerArchetype archetype) {
    switch (arcType) {
      case NarrativeArc.heroicRise:
  ...
  static AchievementNarrative _createDefaultNarrative(
      String serverId, ServerArchetype archetype) {
    return AchievementNarrative(
      ...
      currentMoment:
          "Every expert was once a beginner. This is your moment to start something special.",
  ...
  /// Analyze failure context to determine emotional trigger
  static EmotionalTrigger _analyzeFailureContext(
      String failureContext, ServerArchetype archetype) {
    ...
    } else if (lower.contains('used to') ||
  ...
  static String _generateNostalgiaRecoveryMessage(
      ServerArchetype archetype, String serverId, AppState app) {
    final profile = app.profiles[serverId];
  ...
  static List<String> _generatePrideMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [
  ...
  static List<String> _generateShameMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [
  ...
  static List<String> _generateSuperiorityMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [
  ...
  static List<String> _generateIdentityMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [
  ...
  static List<String> _generatePurposeMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [
  ...
  static List<String> _generateBelongingMessages(
      ServerArchetype archetype, String serverId, AppState app) {
    return [

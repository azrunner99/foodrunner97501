...
class TeamChallenge {
  ...
  double get progressPercentage => targetValue > 0 ? 
    (progress.values.fold(0, (a, b) => a + b) / targetValue).clamp(0.0, 1.0) : 0.0;
...
class SocialCompetitionService {
  ...
  static List<String> generateRivalryMessages(String serverId, AppState app) {
    ...
    if (profile != null && profile.lastKnownRank != null) {
      ...
      if (rankChange > 0) {
        ...
        messages.addAll(_generateDualContextOvertakingMessages(serverId, rankChange, currentRanks, allTimeRanks, careerContext, app));
      } else if (rankChange < 0) {
        // Moved down in ranking - emphasize career vs current disparity
        messages.addAll(_generateDualContextOvertakenMessages(serverId, rankChange.abs(), currentRanks, allTimeRanks, careerContext, app));
      }
    }
    ...
    if (rankDisparity.abs() > 2) {
      messages.addAll(_generateRankDisparityMessages(serverId, serverRank, allTimeRank, app));
    }
    
    // Enhanced leadership/chase messages with career legacy
    if (serverRank == 1) {
      messages.addAll(_generateDualContextLeaderMessages(serverId, allTimeRank, app, currentRanks));
    } else if (serverRank <= 3) {
      messages.addAll(_generateDualContextChaseMessages(serverId, allTimeRank, app, currentRanks));
    }
  ...
  static TeamChallenge createTeamChallenge(
    ...
    switch (type) {
      ...
      case TeamChallengeType.perfectShift:
        ...
        description = "Every team member must maintain their streak for 2 hours!";
  ...
  static void updateTeamChallengeProgress(String serverId, int newRuns, AppState app) {
    for (final challenge in _activeTeamChallenges) {
      if (challenge.participantIds.contains(serverId) && challenge.isActive && !challenge.isExpired) {
        challenge.progress[serverId] = (challenge.progress[serverId] ?? 0) + newRuns;
        
        // Check if challenge completed
        final totalProgress = challenge.progress.values.fold(0, (a, b) => a + b);
  ...
  static void updateStreakCompetition(String serverId, int currentStreak, AppState app) {
    ...
    final messages = _generateStreakRivalryMessages(serverId, currentStreak, app);
  ...
  static Map<String, dynamic> _getCareerContext(ServerProfile profile) {
    ...
    final isConsistent = profile.bestShiftRuns > 0 && profile.allTimeRuns / (profile.bestShiftRuns * 50) > 0.6;
  ...
  static List<String> _generateLeaderMessages(
    ...
    if (secondPlaceId != null) {
      final gap = (app.currentCounts[serverId] ?? 0) - (app.currentCounts[secondPlaceId] ?? 0);
  ...
  static List<String> _generateChaseMessages(
    ...
    if (leaderId != null) {
      final gap = (app.currentCounts[leaderId] ?? 0) - (app.currentCounts[serverId] ?? 0);
  ...
  static List<String> _generateStreakRivalryMessages(
    ...
    final maxOtherStreak = otherStreaks.values.isNotEmpty ? 
      otherStreaks.values.reduce(math.max) : 0;
  ...
  static void _awardTeamCompletionBonuses(TeamChallenge challenge, AppState app) {
  ...
  static List<String> _generateDualContextOvertakingMessages(
    ...
    if (allTimeRank < newRank) {
      ...
      messages.add("📈 CLOSING GAP! Career rank #${allTimeRank} vs current #${newRank} - ${disparity} spots to claim your legacy!");
    }

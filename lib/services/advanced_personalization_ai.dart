...
class LearningMetrics {
  ...
  double get overallAccuracy {
    ...
    return accuracyScores.values.reduce((a, b) => a + b) /
        accuracyScores.length;
  }

  void recordPrediction(String predictionType, bool wasCorrect) {
    predictionCounts[predictionType] =
        (predictionCounts[predictionType] ?? 0) + 1;

    final currentAccuracy = accuracyScores[predictionType] ?? 0.5;
    final weight = 0.1; // Learning rate
    accuracyScores[predictionType] =
        currentAccuracy * (1 - weight) + (wasCorrect ? 1.0 : 0.0) * weight;

    // Track recent performance
    recentPerformance
        .putIfAbsent(predictionType, () => [])
        .add(wasCorrect ? 1.0 : 0.0);
...
class AdvancedPersonalizationAI {
  ...
  static List<BehaviorPrediction> generateBehaviorPredictions(
    ...
    for (final model in PredictionModel.values) {
      final prediction =
          _generatePrediction(serverId, model, behaviorModel, app);
  ...
  static AdaptiveIntervention? deployAdaptiveIntervention(
    ...
    final intervention =
        _createAdaptiveIntervention(serverId, strategy, prediction, app);
  ...
  static void updateLearningAlgorithms(String serverId, AppState app) {
    ...
    for (final intervention in interventions) {
      if (intervention.actualEffectiveness == null) {
        ...
        intervention.actualEffectiveness =
            _measureInterventionEffectiveness(intervention, app);
  ...
  static Map<String, dynamic> generateAddictionStrategy(
      String serverId, AppState app) {
    final profile = app.profiles[serverId];
    if (profile == null) return {};

    final personality = ServerPersonalizationService.analyzeServerPersonality(
        serverId, profile);
    final archetype =
        EmotionalManipulationService.analyzeAndUpdateArchetype(serverId, app);
    final dopamineState =
        NeurochemicalOptimizationService.analyzeDopaminePatterns(serverId);
  ...
  static DateTime? predictOptimalInterventionTime(
      String serverId, AppState app) {
  ...
  static Map<String, dynamic> analyzeLearningPerformance() {
    ...
    for (final entry in _learningMetrics.entries) {
      ...
      analysis[algorithm.toString()] = {
        ...
        'totalPredictions':
            metrics.predictionCounts.values.fold(0, (a, b) => a + b),
  ...
  static Map<String, double> generatePersonalityAdjustments(
      String serverId, AppState app) {
      ...
      if (intervention.wasEffective) {
        ...
        switch (strategy) {
          case InterventionStrategy.competitiveEscalation:
            adjustments['competitiveness'] =
                (adjustments['competitiveness'] ?? 0.0) + 0.1;
            break;
          case InterventionStrategy.socialValidation:
            adjustments['social_motivation'] =
                (adjustments['social_motivation'] ?? 0.0) + 0.1;
            break;
          case InterventionStrategy.personalRecord:
            adjustments['achievement_drive'] =
                (adjustments['achievement_drive'] ?? 0.0) + 0.1;
            break;
          default:
            break;
        }
  ...
  static Map<String, dynamic> _analyzeServerBehavior(
      String serverId, AppState app) {
    final profile = app.profiles[serverId];
    if (profile == null) return {};
    ...
    final performanceTrend =
        _calculateDualContextPerformanceTrend(serverId, app);
    ...
    final shiftVsCareerRatio =
        bestShiftRuns > 0 ? currentRuns / bestShiftRuns : 0.0;
    ...
    return {
      ...
      'careerAveragePerformance':
          allTimeRuns > 0 ? allTimeRuns / 1000.0 : 0.0, // Normalize career
      'recentTrend': performanceTrend,
      'lastActivity': profile.lastTapIso != null
          ? DateTime.parse(profile.lastTapIso!)
          : DateTime.now(),
    ...
    switch (model) {
      case PredictionModel.dropoutRisk:
        final engagementLevel =
            behaviorModel['engagementLevel'] as double? ?? 0.5;
        final recentTrend = behaviorModel['recentTrend'] as double? ?? 0.0;
        final isWorkingToday =
            behaviorModel['isWorkingToday'] as bool? ?? false;
        final careerMomentum =
            behaviorModel['careerMomentum'] as double? ?? 0.5;
        ...
        confidence = (1.0 - engagementLevel) * 0.7 +
            ...
            careerPenalty;
        predictedTime =
            DateTime.now().add(Duration(hours: isWorkingToday ? 1 : 4));
      ...
      case PredictionModel.performanceDecline:
        final performanceTrend =
            behaviorModel['performanceTrend'] as double? ?? 0.0;
        final consistencyScore =
            behaviorModel['consistencyScore'] as double? ?? 0.5;
        final shiftVsCareerRatio =
            behaviorModel['shiftVsCareerRatio'] as double? ?? 0.5;
        ...
        final veteranUnderperformance =
            (careerLevel == 'veteran' || careerLevel == 'legend') &&
                shiftVsCareerRatio < 0.5;

        confidence = performanceTrend < -0.1 ? 0.8 : 0.2;
        if (veteranUnderperformance) confidence += 0.3;

        predictedTime = DateTime.now()
            .add(Duration(minutes: veteranUnderperformance ? 15 : 30));
        ...
        intervention = veteranUnderperformance
            ? InterventionStrategy.prideChallenge
            : InterventionStrategy.personalRecord;
        factors = {
      ...
      case PredictionModel.addictionPeak:
        final dopamineAnalysis =
            NeurochemicalOptimizationService.analyzeDopaminePatterns(serverId);
        final addictionScore =
            dopamineAnalysis['addictionScore'] as double? ?? 0.0;
        ...
        predictedTime =
            DateTime.now().add(Duration(minutes: isShiftActive ? 3 : 8));
  ...
  static AdaptiveIntervention _createAdaptiveIntervention(
    ...
    switch (strategy) {
      case InterventionStrategy.curiosityHook:
        message =
            "🔮 Something mysterious is happening... Are you ready to discover what?";
  ...
  static void _applyInterventionEffects(
      AdaptiveIntervention intervention, AppState app) {
    ...
    switch (intervention.strategy) {
      case InterventionStrategy.competitiveEscalation:
        profile.competitiveLevel =
            (profile.competitiveLevel + 0.1).clamp(0.0, 1.0);
        break;
      case InterventionStrategy.socialValidation:
        profile.socialLevel = (profile.socialLevel + 0.1).clamp(0.0, 1.0);
        break;
      case InterventionStrategy.personalRecord:
        profile.achievementDrive =
            (profile.achievementDrive + 0.1).clamp(0.0, 1.0);
        break;
      default:
        break;
    }
  ...
  static double _measureInterventionEffectiveness(
      AdaptiveIntervention intervention, AppState app) {
    final profile = app.profiles[intervention.serverId];
    if (profile == null) return 0.0;

    final timeSinceIntervention =
        DateTime.now().difference(intervention.deployTime);
    ...
    final recentActivity = profile.lastTapIso != null
        ? DateTime.now()
            .difference(DateTime.parse(profile.lastTapIso!))
            .inMinutes
        : 999;
  ...
  static void _updateLearningMetrics(AdaptiveIntervention intervention) {
    final algorithm =
        LearningAlgorithm.reinforcementLearning; // Default algorithm
    final metrics = _learningMetrics.putIfAbsent(
        algorithm, () => LearningMetrics(algorithm: algorithm));

    final wasSuccessful = intervention.actualEffectiveness! >
        intervention.expectedEffectiveness * 0.8;
  ...
  static void _updatePredictionAccuracy(String serverId, AppState app) {
    ...
    for (final prediction in predictions) {
      final timeSincePrediction =
          DateTime.now().difference(prediction.predictedTime).abs();
      if (timeSincePrediction.inHours < 1) {
  ...
  static bool _checkPredictionAccuracy(
      BehaviorPrediction prediction, AppState app) {
    ...
    switch (prediction.model) {
      ...
      case PredictionModel.addictionPeak:
        final dopamineAnalysis =
            NeurochemicalOptimizationService.analyzeDopaminePatterns(
                prediction.serverId);
  ...
  static double _calculateConsistency(List<DateTime> tapTimes) {
    ...
    for (int i = 1; i < tapTimes.length; i++) {
      intervals
          .add(tapTimes[i].difference(tapTimes[i - 1]).inSeconds.toDouble());
    }
  ...
  static double _calculateSpeedTrend(List<DateTime> tapTimes) {
    ...
    final oldAvg =
        olderIntervals.reduce((a, b) => a + b) / olderIntervals.length;
    final recentAvg =
  ...
  static double _calculateRecentTrend(LearningMetrics metrics) {
    ...
    for (final recent in metrics.recentPerformance.values) {
      if (recent.length >= 3) {
        final first = recent.take(recent.length ~/ 2).reduce((a, b) => a + b) /
            (recent.length ~/ 2);
        final second = recent.skip(recent.length ~/ 2).reduce((a, b) => a + b) /
            (recent.length - recent.length ~/ 2);
  ...
  static double _calculateConfidence(LearningMetrics metrics) {
    ...
    final totalPredictions =
        metrics.predictionCounts.values.reduce((a, b) => a + b);
    final confidence =
        metrics.overallAccuracy * (1.0 - math.exp(-totalPredictions / 10.0));

    return confidence.clamp(0.0, 1.0);
  }

  /// Identify primary psychological hooks
  static List<String> _identifyPrimaryHooks(
      ServerPersonality personality, ServerArchetype archetype) {
    final hooks = <String>[];
  ...
  static List<String> _selectEmotionalTriggers(
      ServerPersonality personality, ServerArchetype archetype) {
  ...
  static Map<String, dynamic> _configureSocialDynamics(
      ServerPersonality personality, AppState app) {
    return {
      'peer_pressure_sensitivity':
          (personality.traitScores[PersonalityTrait.social] ?? 0.5) > 0.7
              ? 0.8
              : 0.4,
      'competitive_messaging': personality.dominantPatterns
              .contains(BehaviorPattern.competitiveShark)
          ? 0.9
          : 0.5,
      'team_dependency':
          (personality.traitScores[PersonalityTrait.social] ?? 0.5) > 0.6
              ? 0.8
              : 0.3,
      'social_validation_need':
          (personality.traitScores[PersonalityTrait.recognition] ?? 0.5) > 0.7
              ? 0.9
              : 0.4,
    };
  }
  ...
  static List<Map<String, dynamic>> _calculateOptimalInterventions(
      String serverId, AppState app) {
    return [
      {
        'type': 'predictive',
  ...
  static double _calculateDualContextPerformanceTrend(
      String serverId, AppState app) {
    final profile = app.profiles[serverId];
    if (profile == null) return 0.0;
    ...
    final currentRuns = app.currentCounts[serverId] ?? 0;
    final bestShiftRuns = profile.bestShiftRuns;
    final allTimeAvg = profile.allTimeRuns > 0
        ? profile.allTimeRuns / 100.0
        : 10.0; // Estimated shifts
  ...
  static double _calculateDualContextEngagementLevel(
      String serverId, AppState app) {
    final profile = app.profiles[serverId];
    if (profile == null) return 0.0;
  ...
  static double _calculateCareerMomentum(ServerProfile profile) {
    final recentPerformance = profile.performanceHistory.isNotEmpty
        ? profile.performanceHistory.last['runs'] as int? ?? 0
        : 0;
    final allTimeAvg =
        profile.allTimeRuns > 0 ? profile.allTimeRuns / 100.0 : 10.0;
    ...
    return ((pointMomentum + performanceMomentum) / 2.0 +
            recentAchievements / 50.0)
        .clamp(0.0, 1.0);
  }

...
class NPSNotificationTester {
  static final NPSNotificationService _notificationService =
      NPSNotificationService();
  ...
  static void generateSampleNotifications() {
    ...
    _notificationService.notifyLowNPSScore(
        sampleServers[1], 8.2, 15); // Critical

    // Generate new feedback notifications
    for (int i = 0; i < 3; i++) {
      _notificationService.notifyNewFeedback(
          sampleFeedback[i], sampleServers[i % 2]);
    }

    // Generate volume notifications
    _notificationService.notifyHighResponseVolume(
        sampleServers[0], 75, 'this week');
    _notificationService.notifyLowResponseVolume(
        sampleServers[1], 3, 'this week');

    // Generate trend notifications
    _notificationService.notifyPerformanceTrend(
        sampleServers[0], 'improved', 12.5, '30 days');
    _notificationService.notifyPerformanceTrend(
        sampleServers[1], 'declined', -8.3, '30 days');
  ...
  static void generateTrendAlert() {
    ...
    _notificationService.notifyPerformanceTrend(
        server, 'declined significantly', -15.2, '7 days');
  }

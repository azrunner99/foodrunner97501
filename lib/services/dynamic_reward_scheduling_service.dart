  ...
  double get nextMultiplier {
    switch (type) {
      ...
      case RewardChainType.dominationChain:
        return math.pow(1.25, currentLevel).toDouble(); // Exponential for domination
...
class DynamicRewardSchedulingService {
  ...
  static List<RewardEvent> checkForVariableRewards(String serverId, AppState app, {
  ...
  static ActiveUrgencyWindow? createUrgencyWindow(UrgencyWindow type, List<String> serverIds) {
  ...
  static void _updateRewardChains(String serverId, String actionType, int baseXP, AppState app) {
    final chains = _serverChains[serverId] ?? <RewardChain>[];
    
    // Check for new chain triggers based on action type
    if (actionType == 'speed_run' && !_hasActiveChain(serverId, RewardChainType.speedChain)) {
      chains.add(RewardChain(serverId: serverId, type: RewardChainType.speedChain));
    } else if (actionType == 'consistent_run' && !_hasActiveChain(serverId, RewardChainType.consistencyChain)) {
      chains.add(RewardChain(serverId: serverId, type: RewardChainType.consistencyChain));
    } else if (actionType == 'comeback_run' && !_hasActiveChain(serverId, RewardChainType.comebackChain)) {
      chains.add(RewardChain(serverId: serverId, type: RewardChainType.comebackChain));
    }
  ...
  static RewardEvent? _generateVariableReward(
    ...
    final careerLevel = profile != null ? _determineCareerLevel(profile.allTimeRuns) : 'rookie';
    final shiftVsCareerRatio = profile != null && profile.bestShiftRuns > 0 ? 
      currentRuns / profile.bestShiftRuns : 0.0;
    ...
    switch (trigger) {
      case VariableRewardTrigger.surpriseBonus:
        ...
        if (careerLevel == 'legend' || careerLevel == 'veteran') {
          message = "👑 VETERAN BONUS! Your ${profile?.allTimeRuns ?? 0} career runs earn ${multiplier}x XP!";
        } else if (shiftVsCareerRatio > 1.2) {
          message = "🚀 BREAKTHROUGH BONUS! Exceeding your standards! ${multiplier}x XP!";
        } else {
      ...
      case VariableRewardTrigger.mysteryMilestone:
        ...
        if (careerLevel == 'legend' && currentRuns > 20) {
          message = "🏆 LEGENDARY MILESTONE! Your mastery unlocks ${multiplier}x secret achievement!";
        } else if (careerLevel == 'rookie' && currentRuns > 10) {
          message = "✨ RISING STAR MILESTONE! Building your legacy! ${multiplier}x secret achievement!";
        } else {
      ...
      case VariableRewardTrigger.jackpotEvent:
        ...
        if (lastJackpot != null && DateTime.now().difference(lastJackpot).inHours < 2) {
        ...
        if (profile != null && profile.allTimeRuns > 1000) {
          message = "💎 CAREER JACKPOT! ${profile.allTimeRuns} runs of experience pays off! ${multiplier}x MEGA REWARD!";
        } else if (isShiftActive && currentRuns > 15) {
          message = "⚡ SHIFT JACKPOT! Peak performance unlocks ${multiplier}x MEGA REWARD!";
        } else {
      ...
      case VariableRewardTrigger.luckyStreak:
        ...
        message = "🍀 LUCKY STREAK! ${multiplier.toStringAsFixed(1)}x fortune bonus!";
        priority = FeedbackPriority.high;
      ...
      case VariableRewardTrigger.flashEvent:
        ...
        message = "⚡ FLASH EVENT! ${multiplier.toStringAsFixed(1)}x lightning bonus!";
        priority = FeedbackPriority.high;
  ...
  static List<String> generateAnticipationMessages(String serverId, AppState app) {
    ...
    if (profile != null) {
      ...
      if (currentRuns > 0 && currentRuns % 7 == 6) { // One before lucky 7 multiple
        messages.add("🎰 JACKPOT BUILDING! One more for a chance at MEGA rewards!");
      }
      
      // Chain advancement teases
      final activeChains = getActiveChains(serverId);
      for (final chain in activeChains) {
        if (chain.currentLevel % 3 == 2) { // About to hit a milestone
          messages.add("🔥 CHAIN LEVEL ${chain.currentLevel + 1} INCOMING! Keep the momentum!");
        }
      }
      
      // Mystery milestone teases
      if (currentRuns > 0 && currentRuns % 11 == 10) { // Prime number patterns
